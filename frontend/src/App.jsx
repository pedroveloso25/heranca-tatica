...
// Copas que devem ser suprimidas da comparação tática (dados insuficientes)
const COPAS_SUPRIMIR = ['1958', '1962', '1974', '1986', '1990']
  ...
  useEffect(() => {
    ...
    Promise.all([comparePromise, historyPromise])
      .then(([compData, histData]) => {
        // Filtrar comparações para remover Copas com dados insuficientes
        if (compData.comparisons) {
          compData.comparisons = compData.comparisons.filter(
            c => !COPAS_SUPRIMIR.includes(c.season)
          )
        }
              ...
              <>
                <div className="bg-slate-800/70 backdrop-blur-sm rounded-2xl p-6 border border-slate-700/50 card-hover">
                  ...
                  <h3 className="text-lg font-semibold text-slate-300 mb-4 flex items-center gap-2">
                    ...
                    Similaridade com edições anteriores
                  </h3>
                  ...
                  {comparison.comparisons.length === 0 ? (
                    <div className="text-center py-8 text-slate-500">
                      ...
                      Apenas uma edição disponível para esta seleção.
                    </div>
                  ) : (
                    <div className="space-y-3">
                      {comparison.comparisons.map((comp) => (
                        <SimilarityBar
                          ...
                          nivelConfiabilidade={comp.nivel_confiabilidade}
                        />
                      ))}
                    </div>
                  )}

                  {/* Aviso sobre Copas ausentes */}
                  {history && history.cobertura.referencia.length > 0 && (
                    <div className="mt-6 p-4 bg-amber-500/10 rounded-xl border border-amber-500/20">
                      <p className="text-sm text-amber-200/80 flex items-start gap-2">
                        ...
                        <span>
                          Dados táticos não disponíveis para <strong>{history.cobertura.referencia.length} Copas</strong>.
                          Veja a aba "Histórico Completo" para detalhes.
                        </span>
                      </p>
                    </div>
                  )}
